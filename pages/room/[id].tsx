...
export default function Room(props: any) {
  ...
  const { getRoomInfo, getUserById, getRequests, getAllRoomsIds, check } =
    ApiServices();
  ...
  const { user, socket, currentRoom, useChatState, chatWindowLoading } =
      ...
      .then(function (registration) {
        const subscribeOptions = {
          ...
          applicationServerKey:
            process.env.NEXT_PUBLIC_SERVICE_WORKER_PUBLIC_KEY ||
            process.env.SERVICE_WORKER_PUBLIC_KEY,
        };
  ...
  const chatSpinner = chatWindowLoading ? (
    <CircularProgress sx={{ margin: "auto" }} />
  ) : null;
  const chatView =
    currentRoom && socket && !chatWindowLoading ? (
      <ChatWindow room={currentRoom} setDrawerOpen={setDrawerOpen} />
    ) : null;
  const noFriends =
    !currentRoom &&
    socket &&
      ...
      >
        ...
        >
          {mobile ? (
            ...
            <div style={{ width: "100%" }}>{t("noFriends")}</div>
          )}
        </div>
      </div>
    ) : null;
  const placeHolder =
    !currentRoom && socket && !chatWindowLoading && user.friends.length > 0 ? (
      <div
        style={{
          display: "flex",
          width: "100%",
      ...
      <ErrorBoundary>
        {pageLoading ? (
          ...
          <div className={classes.roomBoxWrapper}>
            ...
            <div className={classes.chatViewWrapper}>
              {chatSpinner}
              {chatView}
              {placeHolder}
              {noFriends}
