...
const ApiServices = () => {
  ...
  const getRoomInfo = async (id: string) =>
    await postRequest("/rooms/getRoomInfo", { id });

  const getAllRoomsIds = async (idUser: string) =>
    await postRequest("/rooms/getAllRoomsIds", { idUser });

  const uploadImage = async (file: any, id: string) => {
    let data = new FormData();
    data.append("file", file);
    data.append("id", id);
    return await postRequest("/uploadImage", data, {
